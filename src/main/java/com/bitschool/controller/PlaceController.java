...
public class PlaceController {
	...
	public String PlaceRegist(PlaceDTO pl_dto, CompanyDTO cdto, HttpSession session, HttpServletRequest req,
			Model model) {
		String url = null;
		boolean flag = false;
		...
		try {
			...
			Iterator<FileItem> iter = list.iterator(); // list를 FileItem의
														// Iterator로 변환
			FileItem item = null;
			...
			while (iter.hasNext()) { // 이터레이터식 데이터 꺼내기, 다음이 있으면 true
				...
				if (!item.isFormField()) { // isFormField()는 일방적인 데이터 정보를 뜻하고 앞에
											// ! 붙은 !isFormField는 파일정보를 뜻함
					String temp = item.getName();
					int index = temp.lastIndexOf("\\"); // 마지막 \\의 인덱스번호를 구함, 즉,
														// 마지막인덱스 번호를 통해 파일 이름을
														// 뽑겠다.
					...
					} else if (item.getFieldName().equals("placeType")) { // 장소
																			// 유형
						pl_dto.setPlaceType(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeName")) { // 장소
																			// 이름
						pl_dto.setPlaceName(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeIntro")) { // 장소
																			// 소개
						pl_dto.setPlaceIntro(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeAddr1")) { // 주소1,
																			// 우편번호
						pl_dto.setPlaceAddr1(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeAddr2")) { // 주소2,
																			// 도로명
						pl_dto.setPlaceAddr2(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeAddr3")) { // 주소3,
																			// 지번
						pl_dto.setPlaceAddr3(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeAddr4")) { // 주소4,
																			// 상세주소
						pl_dto.setPlaceAddr4(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeTel")) { // 장소
																			// 전화번호
						pl_dto.setPlaceTel(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeURL")) { // 장소
																			// URL
						pl_dto.setPlaceURL(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeUseTime")) { // 이용가능
																				// 시간
					...
					} else if (item.getFieldName().equals("placeCostChoice")) { // 비용
																				// 유형
						pl_dto.setPlaceCostChoice(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeParking")) { // 주차
																				// 유무
						pl_dto.setPlaceParking(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("placeContent")) { // 상세
																				// 설명
			...
			if (fileName != null && !fileName.trim().equals("")) {
				...
				item.write(new File(fileName)); // upload, 즉 저장할 파일의 경로로 item을
												// 저장함
			} else {
				// 파일이름이 널이면 빈값저장
		...
		if (flag) {
			...
			url = "redirect:/place/PlaceListAll"; // 전체 조회 (Reload) : Insert 후,
													// 최신 정보로 업뎃
		} else {
			url = "default";
		}
	...
	public String PlaceModifyProcess(@RequestParam("placeNo") int placeNo, PlaceDTO pl_dto, CompanyDTO cdto,
		...
		// 파일 > 실제 서버에 저장
		String aPath = req.getSession().getServletContext().getRealPath("/"); // 저장할
																				// 파일의
																				// 상대
																				// 경로
		String rPath = "\\resources\\image\\place\\"; // 저장할 파일의 절대 경로
		...
		if (flag) {
			url = "redirect:/place/PlaceDetail?placeNo=" + placeNo; // 수정 완료되면,
																	// 수정된
																	// 상세페이지로
																	// Reload
																	// (수정작업 성공
	...
	// 올린파일을 내부에 저장하는 메서드
	@RequestMapping(value = "/placeImg", method = RequestMethod.POST)
	public @ResponseBody String multiplePhotoUpload(
		...
		try {
			...
			String rPath = "\\resources\\image\\place\\";
			...
			sb.append("&bNewLine=true").append("&sFileName=").append(oldName).append("&sFileURL=")
					.append("http://localhost:5050/resources/image/place/").append(saveName);
	 ...
	 * // 민규. 이건 연습 페이지
	 * 
	 * @RequestMapping(value = "/placeMap", method = {RequestMethod.GET,
	 * RequestMethod.POST}) public String placeMap(Model model) {
	 * 
	...
	/*
	 * 안쓸꺼같음 // 민규. 모집글 쓸때 장소 추천 해주기
	 * 
	 * @RequestMapping(value="/searchPlace", method = {RequestMethod.GET,
	 * RequestMethod.POST}) public @ResponseBody HashMap<String, List<PlaceDTO>>
	 * selectPlace(
	 * 
	 * @RequestParam("keyword") String keyword){
	 * 
	 * System.out.println(keyword); HashMap<String, List<PlaceDTO>> map = new
	 * HashMap<String, List<PlaceDTO>>();
	 * 
	 * List<PlaceDTO> mapList = placeService.searchMap(keyword);
	 * 
	 * if(mapList != null){ map.put("mapList", mapList); }
	 * 
	 * return map; }
	 */
}