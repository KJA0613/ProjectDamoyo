...
<script type="text/javascript">
	function go_url(type) {
		...
		if(query.value === "") {
			var url = "/blog/file/listAll?page=" + page.value;
		} else {
			var url = "/blog/file/search?page=" + page.value;
		}
		if (type == 2) {
			url = "/blog/file/viewModify";
		} else if (type == 3) {
			url = "/blog/file/remove";
		}
		ds.action = url;
		ds.submit();
	}
	...
	function removeReply(type) {
		...
		if(retVal == true) {
			...
			ds.action = "/blog/file/removeReply?blogId=" + blogId + "&blogName=" + blogName;
			ds.submit();
		} else {
		}
	}
...
<body>
	...
	<div id=table_section style="margin-left: 270px; padding-top: 20px;">
		...
		<table class="table" style="width: 700px; margin-left: 50px; border: solid 1px #ccc; background-color: #ffffff">
			...
			<tr style="height: 40px;">
				...
				<form action="" method="POST" id="${reply.replyNo}">
					...
					<input type="hidden" name="boardName" value="file"/>
					<input type="hidden" name="postNo" value="${post.postNo}"/>
					<input type="hidden" name="page" id="page" value="${page}">
				</form>
				<td>
		...
		<c:choose>
		<c:when test = "${null ne pdto.guserId}">
				<form action="/blog/file/reply?blogId=${blogId}&blogName=${blogName}" method="POST" style="margin-top: 70px">
